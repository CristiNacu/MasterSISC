...
)
{
    ...
    UNREFERENCED_PARAMETER(BytesWritten);
    NTSTATUS status;
    PCOMM_CMD_START_MONITORING cmd = (PCOMM_CMD_START_MONITORING)InputBuffer;



    if ((cmd->EnableOptions & PROCESS_NOTIFICATION) && 
        !(gDrv.MonitoringStarted & PROCESS_NOTIFICATION))
    {
        status = ProcFltInitialize();
        if (!NT_SUCCESS(status))
        {
            LogError(L"ProcFltInitialize error, status %llX.\n", status);
            if (cmd->EnableOptions & FAIL_ALL_IF_ERROR)
                return status;
        }
        else
            gDrv.MonitoringStarted |= PROCESS_NOTIFICATION;
    ...
    if ((cmd->EnableOptions & THREAD_NOTIFICATION) && 
        !(gDrv.MonitoringStarted & THREAD_NOTIFICATION))
    {
    ...
    if ((cmd->EnableOptions & IMAGE_NOTIFICATION) && 
        !(gDrv.MonitoringStarted & IMAGE_NOTIFICATION))
    {
    ...
    if ((cmd->EnableOptions & REGISTRY_NOTIFICATION) && 
        !(gDrv.MonitoringStarted & REGISTRY_NOTIFICATION))
    {
        status = RegFltInitialize(gDrv.DriverObject);
        if (!NT_SUCCESS(status))
        {
            LogError(L"ProcFltInitialize error, status %llX.\n", status);
            if (cmd->EnableOptions & FAIL_ALL_IF_ERROR)
    ...
    if ((cmd->EnableOptions & FILE_NOTIFICATION) && 
        !(gDrv.MonitoringStarted & FILE_NOTIFICATION))
    {
...
)
{
    ...
    UNREFERENCED_PARAMETER(BytesWritten);
    NTSTATUS status;
    PCOMM_CMD_STOP_MONITORING cmd = (PCOMM_CMD_STOP_MONITORING)InputBuffer;


    if (cmd->DisableOptions & BIT(PROCESS_NOTIFICATION) &&
        (gDrv.MonitoringStarted & PROCESS_NOTIFICATION))
    {
        status = ProcFltInitialize();
        if (!NT_SUCCESS(status))
    ...
    if (cmd->DisableOptions & BIT(THREAD_NOTIFICATION) &&
        (gDrv.MonitoringStarted & THREAD_NOTIFICATION))
    {
        status = ThreadFltInitialize();
        if (!NT_SUCCESS(status))
    ...
    if (cmd->DisableOptions & BIT(IMAGE_NOTIFICATION) &&
        (gDrv.MonitoringStarted & IMAGE_NOTIFICATION))
    {
        status = ImgFltInitialize();
        if (!NT_SUCCESS(status))
    ...
    if (cmd->DisableOptions & BIT(REGISTRY_NOTIFICATION) &&
        (gDrv.MonitoringStarted & REGISTRY_NOTIFICATION))
    {
        status = RegFltInitialize(gDrv.DriverObject);
        if (!NT_SUCCESS(status))
    ...
    if (cmd->DisableOptions & BIT(FILE_NOTIFICATION) &&
        (gDrv.MonitoringStarted & FILE_NOTIFICATION))
    {
        status = FltStartFiltering(gDrv.FilterHandle);
        if (!NT_SUCCESS(status))
...
)
{
    ...
    __try
    {
        ...
        {
            LogTrace("Sending message to app. Input: 0x%p size %u Output: Not required",
...
_IRQL_requires_max_(APC_LEVEL)
NTSTATUS
CommSendUnicodeString(
    _In_ PUNICODE_STRING String
)
{
    NTSTATUS status;
    ULONG stringreplySize = 0;

    PMY_DRIVER_MSG_STRING_NOTIFICATION msgStringNotification =
        (PMY_DRIVER_MSG_STRING_NOTIFICATION)ExAllocatePoolWithTag(PagedPool, sizeof(PMY_DRIVER_MSG_STRING_NOTIFICATION) + String->Length, '#SSS');

    if (!msgStringNotification)
    {
        LogError("Failed to allocate PMY_DRIVER_MSG_STRING_NOTIFICATION structure!\n");
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    msgStringNotification->Header.MessageCode = msgSendString;
    msgStringNotification->Length = String->Length;
    RtlCopyMemory(msgStringNotification->Data, String->Buffer, String->Length);


    status = CommSendMessage(String->Buffer, sizeof(PMY_DRIVER_MSG_STRING_NOTIFICATION) + String->Length, NULL, &stringreplySize);
    if (!NT_SUCCESS(status))
    {
        LogError("CommSendMessage returned error\n");
    }

    ExFreePoolWithTag(msgStringNotification, '#SSS');
    return status;
}